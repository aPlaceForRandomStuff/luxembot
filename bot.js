...
function addUser(message, args) {
    // Making sure that only mods can run the !add command
    if (!message.member.roles.find("id", config.roles.mods)) {
      console.log("Non-moderator attempted to add new member.");
      return;
    }
    if (message.channel.id != config.channels.lobby) {
      console.log("Command to add user was run in the wrong channel.");
      return;
    }
    //Add the mentioned user to the member role, change their nickname, and ping them with a welcome message
    const role = message.guild.roles.find("id", config.roles.members);
    const member = message.mentions.members.first();
    member.addRole(role).catch(console.error);
    member.setNickname(`Comrade ${member.displayName}`);
...
client.on("message", (message) => {
    ...
    } else if (command == "setroles" && message.channel.id == config.channels.mod) { //These commands allow the mods to control the role options that members have
        if (args[0]) {
